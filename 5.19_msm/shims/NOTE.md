...
* **Zap shader init:** **FIXED.** On downstream you need `pil_gpu` enabled because that's how the TrustZone driver probes pas-id XX and authenticates the zap at boot.
* **Current issue(s):** GPU Scheduler needs to be backported from 5.19. 🙃 (I'll likely also do something about the drm_syncobj thing.)
