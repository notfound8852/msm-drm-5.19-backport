...
#if IS_MODULE(CONFIG_DRM_MSM)
#include <linux/pm_domain.h>

/*
 * PROBLEM: Runtime module loading.
 *
 * When built-in, the driver probes right after bootloader hand-off, in which case the power
 * domains are enabled. Conversely, when the driver is loaded via insmod from userspace,
 * power domains like GPU_CX_GDSC and MDSS_GDSC may have already transitioned to an off state.
 * We need to either raise a temporary runtime PM vote or manually enable the power domain(s)
 * so that the underlying hardware rails and AHB clock are operational before we touch any
 * clocks or registers.
 *
 * NOTE: Standard runtime PM APIs (e.g., pm_runtime_get_sync) cannot be used safely in any early
 * initialization case, as doing so would trigger the runtime PM resume path. In the case of MDSS,
 * it triggers mdss_runtime_resume -> msm_mdss_enable, which enables clocks and attempts register
 * reads (readl_relaxed) on an uninitialized MDSS structure, resulting in a fatal error.
 * Instead, we manually invoke the genpd provider to force-enable the power domain
 * ONCE inside msm_mdss_init before returning the initialized struct. This emulates the bootloader
 * hand-off behavior.
 */

inline int dev_gdsc_enable(struct platform_device *pdev) {
    ...
    if (genpd) {
        ...
        if (ret) {
            dev_err(&pdev->dev, "Failed to force push GDSC power: %d\n", ret);
            return ret;
        }
    }
    return 0;
...
#else
        unsigned int mapping = irq_find_mapping(msm_mdss->irq_controller.domain,
                       hwirq);
        if (mapping == 0) {
