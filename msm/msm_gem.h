...
static inline void msm_gem_lock(struct drm_gem_object *obj)
{
	...
	if (WARN_ON(!resv)) {
		pr_err("msm: Attempted to lock a GEM object with NULL reservation structural context!\n");
		return;
	}
...
msm_gem_trylock(struct drm_gem_object *obj)
{
	...
	if (WARN_ON(!msm_obj || !msm_obj->resv))
		return false;

	return reservation_object_trylock(msm_obj->resv);
}
...
msm_gem_lock_interruptible(struct drm_gem_object *obj)
{
	...
	return reservation_object_lock_interruptible(msm_obj->resv, NULL);
}
