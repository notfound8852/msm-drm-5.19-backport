                                 ...
                                 bool quiet, phys_addr_t *psize)
{
    ...
    // If looking up by name, find its index first
    if (name) {
        index = of_property_match_string(pdev->dev.of_node, "reg-names", name);
        if (index < 0) {
            if (!quiet)
                DRM_DEV_ERROR(&pdev->dev, "failed to find reg-name: %s\n", name);
            return ERR_PTR(-EINVAL);
        }
    }

    // This native OF call handles #address-cells and #size-cells translation automatically
    ret = of_address_to_resource(pdev->dev.of_node, index, &res);
    if (ret) {
        if (!quiet)
            DRM_DEV_ERROR(&pdev->dev, "failed to get memory resource for index %d: %s\n", index, name);
        return ERR_PTR(ret);
    }

    size = resource_size(&res);

    // Use standard devm_ioremap with the translated resource boundaries
    ptr = devm_ioremap(&pdev->dev, res.start, size);
    if (!ptr) {
        if (!quiet)
            DRM_DEV_ERROR(&pdev->dev, "failed to ioremap: %s\n", name);
