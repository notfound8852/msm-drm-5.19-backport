...
#else
        drm_writeback_queue_job(wb_enc->wb_conn, state->writeback_job);
#endif
		...
		struct drm_writeback_job *job)
{
...
#else
    {
        ...
        drm_connector_list_iter_begin(phys_enc->dpu_kms->dev, &conn_iter);
        drm_for_each_connector_iter(connector, &conn_iter) {
            if (connector->encoder == phys_enc->parent) {
                wb_enc->wb_conn = drm_connector_to_writeback(connector);
                break;
            }
        }
