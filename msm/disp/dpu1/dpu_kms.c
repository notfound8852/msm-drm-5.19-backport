...
static int dpu_kms_parse_data_bus_icc_path(struct dpu_kms *dpu_kms)
{
    struct drm_device *dev = dpu_kms->dev;
    int ret;
    /* decently simple usage */
    ret = of_icc_get(dev->dev, dpu_kms->path, &dpu_kms->num_paths);
    return ret;
}
#endif
...
static int _dpu_kms_mmu_init(struct dpu_kms *dpu_kms)
{
	 ...
	 */
	domain = iommu_get_domain_for_dev(mdss_dev);

	if (!domain) {
		DPU_DEBUG("No existing IOMMU domain found, allocating one\n");
		domain = iommu_domain_alloc(&platform_bus_type);
	} else {
		DPU_DEBUG("Using existing IOMMU domain associated with MDSS\n");
	}
...
static int __maybe_unused dpu_runtime_suspend(struct device *dev)
{
	...
	/**
	 * NOTE: If the interconnector shim is being used
	 * here the voltage is taken care of implicitly here.
	 */
