...
static irqreturn_t a6xx_hfi_irq(int irq, void *data)
{
    ...
    {
        status = gmu_read(gmu, REG_A6XX_GMU_AO_HOST_INTERRUPT_STATUS);
        pr_info("we legitametly can read and write fine. %llu", status);
    }
        ...
        unsigned long *freqs, int freqs_count, const char *id)
{
	...
	u16 pri[32] = {0};
	u16 sec[32] = {0};
	size_t pri_len, sec_len;
	size_t pri_count, sec_count;

	pri_len = cmd_db_read_aux_data_len(id);
	if (pri_len == 0 || pri_len > sizeof(pri))
		return -EINVAL;

	cmd_db_read_aux_data(id, (u8 *)pri, pri_len);
	pri_count = pri_len >> 1; // 2 bytes per u16 element

	/* 2. Read Secondary ARC data ("mx.lvl") */
	sec_len = cmd_db_read_aux_data_len("mx.lvl");
	if (sec_len == 0 || sec_len > sizeof(sec))
		return -EINVAL;

	cmd_db_read_aux_data("mx.lvl", (u8 *)sec, sec_len);
	sec_count = sec_len >> 1;
	...
	for (i = 0; i < freqs_count; i++) {
		...
		/* Look for a level in the secondary list that matches */
		for (j = 0; j < sec_count; j++) {
			if (sec[j] >= level) {
				sindex = j;
				break;
		...
		/* Construct the vote exactly how the GMU firmware expects it */
		votes[i] = ((pri[pindex] & 0xffff) << 16) | (sindex << 8) | pindex;
	}
...
int a6xx_gmu_init(struct a6xx_gpu *a6xx_gpu, struct device_node *node)
{
	...
	/*
	 * PATCH: Late-stage runtime module loading workaround. Refer to `msm_mdss.c`
	 * For the full reasoning and function.
	 */
	...
	/*
	 * PATCH: Handle the CX domain as well as the GX domain via the MSM driver.
	 */
